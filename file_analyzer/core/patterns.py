...
def get_patterns() -> Dict[str, str]:
    ...
    """
    return {
        # moved ipv4/ipv6/domain/url/va.gov detections to endpoints plugin
        # moved mac_address to endpoints plugin
        ...
        # Require more substantial value; avoid common placeholders
        ...
        'password': r"(?i)\b(?:password|pass|pwd|secret)\b\s*[:=]\s*['\"]((?!changeme|example|sample|test|dummy|password|secret|redacted)['\"\S]{4,})['\"]",
        ...
        'access_token': r"(?i)\b(?:access[_-]?token|bearer[_-]?token)\b\s*[:=]\s*['\"]((?!example|sample|test|dummy|redacted)['\"A-Za-z0-9._-]{16,})['\"]",
        'refresh_token': r"(?i)\brefresh[_-]?token\b\s*[:=]\s*['\"]((?!example|sample|test|dummy|redacted)['\"A-Za-z0-9._-]{16,})['\"]",
        'oauth_token': r"(?i)\boauth[_-]?token\b\s*[:=]\s*['\"]((?!example|sample|test|dummy|redacted)['\"A-Za-z0-9._-]{16,})['\"]",
...
def get_endpoint_patterns() -> Dict[str, str]:
    """Endpoint-related patterns: IPs, domains, URLs, VA.gov specifics.

    These were moved out of the main pattern set to the endpoints plugin
