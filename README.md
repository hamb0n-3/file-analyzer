...
file-analyzer file ./a.py ./b.js --plugin code,api --html results.html

# Analyze a directory non-recursively and export aggregated per-plugin reports
file-analyzer dir ./project --plugin all --html --json --csv --output-dir ./out
# Produces: out/plugin-api.html, out/plugin-endpoints.json, out/plugin-code.csv, out/plugin-secrets.json, ...
...
file-analyzer dir ./project --plugin code,endpoints --html --output-dir ./out
...
- `--plugin <list>`: Comma-separated plugin groups: `code, api (web), endpoints (network), json, xml, secrets, crypto, ml, all`.
...
The JSON and XML parsers run regardless of `--plugin` selection. They do not perform security analysis; instead they aid other plugins by parsing structure and exposing minimal metadata:
...
- `plugin-secrets.(json|html|csv)`
- plus `summary.html` and `summary.json` (directory overview)

Secret findings are included in the aggregated per-plugin outputs when `--plugin secrets` (or `all`) is selected.
...
- Use `--plugin` to restrict work to what you need (e.g., `code,api`).
